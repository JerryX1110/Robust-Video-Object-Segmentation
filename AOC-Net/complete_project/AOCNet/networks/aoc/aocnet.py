...
from networks.layers.matching import global_matching, global_matching_for_eval, local_matching, foreground2background, global_matching_proxy,local_matching_proxy,global_matching_cluster2, global_matching_cluster,global_matching_for_eval_cluster, global_matching_for_eval_proxy
from networks.layers.attention import calculate_attention_head, calculate_attention_head_for_eval, calculate_attention_head_p_m, calculate_attention_head_for_eval_p_m
from networks.p2t.decoding_module import  CalibrationDecoding,DynamicPreHead


class AOCNet(nn.Module):
    def __init__(self, cfg, feature_extracter):
        super(AOCNet, self).__init__()
        self.cfg = cfg
        ...
        self.dynamic_seghead = CalibrationDecoding(
        ...
        in_dim = 2*(2 + len(cfg.MODEL_MULTI_LOCAL_DISTANCE))-1+2
    ...
    def before_seghead_process(self, memory_prev_list=None,
        ...
        for n in range(bs):
            ...
            ## start global matching cluster (defualt num: 16)
            if self.training:
                seq_ref_frame_embedding = ref_frame_embedding[n]
                seq_ref_frame_embedding = seq_ref_frame_embedding.permute(1,2,0)
                ...
                global_matching_fg_cluster = global_matching_cluster2(
                    reference_embeddings=seq_ref_frame_embedding, 
                    query_embeddings=seq_current_frame_embedding, 
                    reference_labels=seq_ref_frame_label, 
                    n_chunks=cfg.TRAIN_GLOBAL_CHUNKS, 
                ...
                global_matching_fg_cluster = global_matching_for_eval_cluster(
                    all_reference_embeddings=all_reference_embeddings,
                    query_embeddings=seq_current_frame_embedding, 
                    all_reference_labels=all_reference_labels,
                    n_chunks=cfg.TEST_GLOBAL_CHUNKS,
            ...
            ## end global matching cluster  (defualt num: 16)
                
            ## start local matching
            ...
            ## start refereced instance-level object representation calculation (proxy indicates cluster_num = 1)
            if self.training:
                ...
                attention_head = calculate_attention_head_for_eval(
                    ...
                    to_cat_previous_frame, 
                    epsilon=self.epsilon)
            ## end refereced instance-level object representation calculation (proxy indicates cluster_num = 1)


            ## start proxy-based matching
            if self.training:
                attention_head,ref_head_pos, ref_head_neg, prev_head_pos, prev_head_neg= calculate_attention_head_p_m(
                    ref_frame_embedding[n].unsqueeze(0).expand((obj_num,-1,-1,-1)),
                    to_cat_ref_frame, 
                    previous_frame_embedding[n].unsqueeze(0).expand((obj_num,-1,-1,-1)), 
                    to_cat_previous_frame,
                ...
                attention_head,ref_head_pos, ref_head_neg, prev_head_pos, prev_head_neg  = calculate_attention_head_for_eval_p_m(
                    ref_frame_embedding,
                    seq_ref_frame_labels,
                    previous_frame_embedding[n].unsqueeze(0).expand((obj_num,-1,-1,-1)), 
                    to_cat_previous_frame, 
            ...
            if self.training:
                global_matching_fg_proxy = global_matching_proxy(
                    reference_embeddings=ref_head_pos, 
                    query_embeddings=seq_current_frame_embedding, 
                    reference_labels=seq_ref_frame_label, 
                    n_chunks=cfg.TRAIN_GLOBAL_CHUNKS, 
                    dis_bias=dis_bias, 
                ...
                global_matching_fg_proxy = global_matching_for_eval_proxy(
                    all_reference_embeddings=ref_head_pos,
                    query_embeddings=seq_current_frame_embedding, 
                    all_reference_labels=all_reference_labels,
                    n_chunks=cfg.TEST_GLOBAL_CHUNKS,
                    dis_bias=dis_bias, 
            ...
            seq_prev_frame_embedding_inst = torch.matmul(seq_previous_frame_label, prev_head_pos)


            local_matching_fg_proxy = local_matching_proxy(
                prev_frame_embedding=seq_prev_frame_embedding_inst, 
                query_embedding=seq_current_frame_embedding, 
                prev_frame_labels=seq_previous_frame_label,
                multi_local_distance=cfg.MODEL_MULTI_LOCAL_DISTANCE,
                dis_bias=dis_bias, 
            ...
            to_cat_global_matching_fg_proxy = global_matching_fg_proxy.squeeze(0).permute(2,3,0,1)
            to_cat_global_matching_fg_cluster = global_matching_fg_cluster.squeeze(0).permute(2,3,0,1)
            to_cat_global_matching_fg = global_matching_fg.squeeze(0).permute(2,3,0,1)
            to_cat_local_matching_fg_proxy = local_matching_fg_proxy.squeeze(0).permute(2,3,0,1)
            ...
            pre_to_cat = torch.cat((to_cat_global_matching_fg, to_cat_global_matching_fg_cluster, to_cat_global_matching_fg_proxy,to_cat_local_matching_fg, to_cat_local_matching_fg_proxy, to_cat_previous_frame), 1)
