...
import numpy as np
from dataloaders.datasets_m import YOUTUBE_VOS_Test,DAVIS_Test
import dataloaders.custom_transforms as tr
from networks.deeplab.deeplab import DeepLab
from utils.meters import AverageMeter
from utils.image import flip_tensor, save_mask, save_matching_result
...
class Evaluator(object):
    ...
    def prepare_dataset(self):
        ...
        if len(cfg.TEST_MULTISCALE) > 1:
            eval_name += '_ms'
            for ss in cfg.TEST_MULTISCALE:
                eval_name +="_"
                eval_name +=str(ss)

        eval_name+="_m_"+str(self.mem_every)+"_u_"+str(self.unc_ratio)+"_r_"+str(cfg.TEST_MAX_SIZE)+"_RPA"
        ...
        if cfg.TEST_DATASET == 'youtubevos19':
            ...
            self.dataset = YOUTUBE_VOS_Test(
                ...
                transform=eval_transforms,  
                result_root=self.result_root)
        elif cfg.TEST_DATASET == 'youtubevos':  ## predict all frame for youtubevos 2019
            self.result_root = os.path.join(cfg.DIR_EVALUATION, cfg.TEST_DATASET, eval_name, 'Annotations')
            self.dataset = YOUTUBE_VOS_Test(
                root=cfg.DIR_YTB_EVAL, 
                transform=eval_transforms,  
            ...
            self.dataset = DAVIS_Test(
                ...
                result_root=self.result_root)
        elif cfg.TEST_DATASET == 'davis2017-label':
            resolution = 'Full-Resolution' if cfg.TEST_DATASET_FULL_RESOLUTION else '480p'
            self.result_root = os.path.join(cfg.DIR_EVALUATION, cfg.TEST_DATASET, eval_name, 'Annotations', resolution)
            self.dataset = DAVIS_Test_w_label(
                split=cfg.TEST_DATASET_SPLIT, 
                root=cfg.DIR_DAVIS, 
                year=2017, 
                transform=eval_transforms,
            ...
            with torch.no_grad():
                for frame_idx, samples in enumerate(seq_dataloader):
                    ...
                    for aug_idx in range(len(samples)):
                        ...
                        # delete the label that hasn't existed in the GT label
                        all_pred_remake = []
                        all_pred_exist = []
                        if all_pred!=None:
