...
class Refiner(nn.Module):
    ...
    def __init__(self,
        ...
        channels = [128, 64, 32, 16, 1] #4->1
        self.conv1 = nn.Conv2d(channels[0]  + 4, channels[1], kernel_size, bias=False)  # 6->3
    ...
    def forward(self,
                src: torch.Tensor,   #image
                #bgr: torch.Tensor,
                pha: torch.Tensor,   #probability_map (B, 1, H, W)
                #fgr: torch.Tensor,   #warp_result
                err: torch.Tensor,   #uncertainty map (B, 1, H, W)
                hid: torch.Tensor):  #feature    
        ...
        src_bgr =  src  #torch.cat([src, bgr], dim=1)
        
        if self.mode != 'full':
            err = F.interpolate(err, (H_quat, W_quat), mode='bilinear', align_corners=False) # downsample error map to 1/4
            ref = self.select_refinement_regions(err) #select top error regions 
            idx = torch.nonzero(ref.squeeze(1))
            idx = idx[:, 0], idx[:, 1], idx[:, 2]
            
            if idx[0].size(0) > 0:
                x = torch.cat([hid, pha], dim=1) #32 + 1 
                ...
                y = F.interpolate(src_bgr, (H_half, W_half), mode='bilinear', align_corners=False) # 6->3
                ...
                out = pha # torch.cat([pha, fgr], dim=1)
