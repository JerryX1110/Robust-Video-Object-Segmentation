...
def cal_shanoon_entropy(preds):  # (batch, obj_num, 128, 128)
...
def cal_shanoon_entropy_train(preds):  # (batch, obj_num, 128, 128)
    uncertainty = -1.0 * torch.sum(preds * torch.log(preds + 1e-6), dim=1, keepdim=True)  # (batch, 1, 128, 128)
...
def show_shanoon_entropy(uncertainty,uncertainty_norm,unc_rate=0.5):   #torch.tensor, torch.tensor
...
def save_shanoon_entropy(uncertainty,uncertainty_norm,save_path,unc_rate=0.5):   #torch.tensor, torch.tensor
    uncertainty = uncertainty.cpu().numpy().squeeze().astype('float32')
    uncertainty = uncertainty * (uncertainty > unc_rate)
    uncertainty_norm = uncertainty_norm.cpu().numpy().squeeze().astype('float32')
    uncertainty_norm = uncertainty_norm
...
def save_shanoon_entropy_calculated(uncertainty,save_path,unc_rate=1):   #torch.tensor, torch.tensor
    uncertainty = uncertainty.cpu().numpy().squeeze().astype('float32')
