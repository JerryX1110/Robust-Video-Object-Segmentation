...
class VOSTest(Dataset):
    ...
    def read_image_and_add_gaussian_noise(self, idx, gaussian_noise_std):
        img_name = self.images[idx]
        img_path = os.path.join(self.image_root, self.seq_name, img_name)
        img = cv2.imread(img_path)
        img = np.array(img, dtype=np.float32)

        gnoise = np.zeros(img.shape,img.dtype)
        m=(0,0,0)  
        s=(gaussian_noise_std,gaussian_noise_std,gaussian_noise_std) 
        cv2.randn(gnoise,m,s)
        img = cv2.add(img,gnoise)

        if self.rgb:
            img = img[:, :, [2, 1, 0]]
        return img
    
    def read_image_and_add_gaussian_blur(self, idx, blur_kernel_size):  #9
        img_name = self.images[idx]
        img_path = os.path.join(self.image_root, self.seq_name, img_name)
        img = cv2.imread(img_path)
        img = np.array(img, dtype=np.float32)
    ...
    def read_image_and_add_salt_and_pepper_noise(self, idx, perturbed_points_num):
        img_name = self.images[idx]
        img_path = os.path.join(self.image_root, self.seq_name, img_name)
        img = cv2.imread(img_path)
        img = np.array(img, dtype=np.float32)
        ...
        nums = perturbed_points_num
        rows = np.random.randint(0, h, (perturbed_points_num), dtype = np.int)
        cols = np.random.randint(0,w,(perturbed_points_num),dtype = np.int)
        for i in range(nums):
            if i%2 == 1:
    ...
    def __getitem__(self, idx):
        ...
        if self.image_type==0:
            ...
            # read_image_and_add_gaussian_noise_with_std=5
            current_img = self.read_image_and_add_gaussian_noise(idx)
        elif self.image_type==2:
            # read_image_and_add_gaussian_noise_with_std=10
            current_img = self.read_image_noise2(idx)
        elif self.image_type==3:
            # read_image_and_add_gaussian_noise_with_std=30
            current_img = self.read_image_and_add_blur(idx, 3)
        elif self.image_type==4:
            # read_image_and_add_gaussian_blur_with_kernel=3
            current_img = self.read_image_and_add_blur(idx, 3)
        elif self.image_type==5:
            # read_image_and_add_gaussian_blur_with_kernel=5
            current_img = self.read_image_and_add_blur(idx, 5)
        elif self.image_type==6:
            # read_image_and_add_gaussian_blur_with_kernel=9
            current_img = self.read_image_and_add_blur(idx, 9)
        elif self.image_type==7:
            # read_image_and_add_salt_and_pepper_noise_with_1000_points
            current_img = self.read_image_and_add_salt_and_pepper_noise(idx, 1000)
        elif self.image_type==8:
            # read_image_and_add_salt_and_pepper_noise_with_1000_points
            current_img = self.read_image_and_add_salt_and_pepper_noise(idx, 1000)
        elif self.image_type==9:
            # read_image_and_add_salt_and_pepper_noise_with_5000_points
            current_img = self.read_image_and_add_salt_and_pepper_noise(idx, 5000)
        ...
        if current_label_name in self.labels and idx==0:
...
class YOUTUBEVOS_Test(object):
    def __init__(self,
        ...
        if split == 'val':
            split = 'valid'
        root = os.path.join(root, split)# str(year)
        self.db_root_dir = root
        self.image_type = image_type
        self.result_root = result_root
        self.rgb = rgb
        self.transform = transform
        print("self.seq_list_file",os.path.join(self.db_root_dir, 'meta.json'))
...
class DAVIS_Test(object):
    def __init__(self,
        ...
        if label_selection in [8,9] :
            root = "/run/rvos_data_all_633/"
            print("root",root)
        elif int(label_selection)>1000:
            root = "/dev/shm/rvos_test_"+str(int(label_selection-1000))
        elif int(label_selection)>100:
            root = "/dev/shm/rvos_test_"+str(int(label_selection-0))
        ...
        print("self.label_root",self.label_root)
        seq_names = []
        for spt in split:
            if spt == 'test':
                spt = 'test-dev'
        ...
        if int(label_selection)>1000:
            inputt = open(os.path.join(root,"ref_"+str(360)+".pkl"),"rb")
        else:
            inputt = open(os.path.join(root,"ref_"+str(360)+".pkl"),"rb")

        import pickle
        dddddd = pickle.load(inputt)
        self.dddddd = dddddd
        #print("len(self.dddddd.keys())",len(self.dddddd.keys()))
