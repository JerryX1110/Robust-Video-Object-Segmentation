...
class VOS_Test(Dataset):
    def __init__(self, image_root, label_root, seq_name, images, labels, rgb=False, transform=None, single_obj=False, resolution=None, image_type=0):
        self.image_root = image_root
        self.label_root = label_root
        self.seq_name = seq_name
        self.images = images
...
class YOUTUBE_VOS_Test(object):
    def __init__(self, root='./valid', transform=None, rgb=False, result_root=None,use_all=False,image_type=0):
        self.image_type = image_type
    ...
    def __getitem__(self, idx):
        ...
        images = np.sort(np.unique(images))
        labels = np.sort(np.unique(labels))
        seq_dataset = VOS_Test(self.image_root, self.label_root, seq_name, images, labels, transform=self.transform, rgb=self.rgb,image_type = self.image_type)
        return seq_dataset
